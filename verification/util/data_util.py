...
class MisinformationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if img_list != []:
            ...
            inputs = self.processor(text=claim_and_text_evidence_list, images=img_list, return_tensors="pt",
                                    padding=True, truncation=True, max_length=77)
        else:
            inputs = self.processor(text=claim_and_text_evidence_list, return_tensors="pt", padding=True,
...
def get_data_loader(train_data_dir, train_img_dir, val_data_dir, val_img_dir, test_data_dir, test_img_dir, processor,
                    tokenizer):
    test_ids, test_claims, test_evidences, test_images, test_labels, test_label_statistic = get_data(test_data_dir,
                                                                                                     test_img_dir)
    val_ids, val_claims, val_evidences, val_images, val_labels, val_label_statistic = get_data(val_data_dir,
                                                                                               val_img_dir)
    train_ids, train_claims, train_evidences, train_images, train_labels, train_label_statistic = get_data(
        train_data_dir, train_img_dir)
    train_dataset = MisinformationDataset(train_claims, train_evidences, train_images, train_labels, train_img_dir,
                                          processor, tokenizer, train_ids)
    val_dataset = MisinformationDataset(val_claims, val_evidences, val_images, val_labels, val_img_dir, processor,
                                        tokenizer, val_ids)
    test_dataset = MisinformationDataset(test_claims, test_evidences, test_images, test_labels, test_img_dir, processor,
                                         tokenizer, test_ids)
