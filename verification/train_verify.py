...
def setup_args_for_verify(config_kwargs):
    ...
    args.train_img_dir = os.path.join(args.train_dir, "images")
    args.val_img_dir = os.path.join(args.val_dir, "images")
    args.test_img_dir = os.path.join(args.test_dir, "images")
    return args, logger
...
def train_loop(tuner_config, config_kwargs):
    ...
    model = MultiModal2(bert=None, clip=clip_model, concat_hidden_size=256, final_hidden_size=64,
                        num_classes=3, max_image_num=args.max_image_num, model_type=args.model_type,
                        model_type_enum=model_type_enum,
                        is_project_embed=model_type_enum.is_project_embed,
                        only_claim_setting=model_type_enum.only_claim_setting,
    ...
    train(model, dataloaders, device, optimizer, args.max_image_num, args, train_label_statistic, logger,
          args.batch_size)
...
def train(model, dataloaders, device, optimizer, max_image_num, args, train_label_statistic, logger, batch_size):
    ...
    if args.is_wandb == "y":
        wandb.login(key="fd3205c4cff7bb4deb4b4e5608209261f51ec103")
...
def test_phase(accum_iter, model, dataloaders, device, args, logger, epoch, best_epoch):
    ...
    epoch_avg_acc, f1_score, pre, recall, best_valid_f1, best_epoch, is_early_stop = val_phase(accum_iter, model,
                                                                                               dataloaders["Test"],
                                                                                               device, np.inf, args,
                                                                                               args.save_predict, epoch,
                                                                                               best_epoch)
    wandb.log({"test_f1": f1_score})
    logger.write(
        "F1: {:.4f}, Precision: {:.4f}, Recall : {:.4f}, Accuracy: {:.4f}".format(f1_score, pre, recall, epoch_avg_acc))
...
def calc_metric(y_true, y_pred, args):
    ...
    if args.verbos == "y":
        ...
        wandb.log({"by_label/refuted_f1": c_report["Refuted"]["f1-score"],
                   "by_label/supported_f1": c_report["Supported"]["f1-score"],
                   "by_label/nei_f1": c_report["NEI"]["f1-score"]})
