...
class MultiModal2(torch.nn.Module):
    ...
    def __init__(self, bert, clip, concat_hidden_size, final_hidden_size, num_classes, max_image_num, model_type,
                 model_type_enum, is_project_embed, only_claim_setting, token_pooling_method):
        """
        In the constructor we instantiate two nn.Linear modules and assign them as
        member variables.
        """
    ...
    def forward(self, claim_img_encod, device):
        ...
        if self.is_project_embed == "y":
            ...
            if has_image:
                ...
                img_evidence_mask = torch.ones(img_evidences_embed.shape[0], img_evidences_embed.shape[1],
                                               dtype=img_evidences_embed.dtype, device=img_evidences_embed.device)
            else:
                img_evidences_embed = None
                img_evidence_mask = None
                text_model_output = self.clipmodel.text_model(**claim_img_encod)
        ...
        output = self.verifier(self.has_evidence(has_image), claim_embed, txt_evidences_embed, img_evidences_embed,
                               has_image, claim_mask, txt_evidence_mask, img_evidence_mask)
        return output

    def has_evidence(self, has_image):
        if not self.model_type_enum.use_text_evidence and (
                not self.model_type_enum.use_image_evidence or not has_image):
            return False
        else:
            return True
...
class Verifier(torch.nn.Module):
    ...
    def forward(self, has_evidence, claim_embed, txt_evidences_embed, img_evidences_embed, has_image, claim_mask,
                txt_evidence_mask, img_evidence_mask):
        if has_evidence:
            output = self.stance_detect_layer(claim_embed, txt_evidences_embed, img_evidences_embed, has_image,
                                              claim_mask, txt_evidence_mask, img_evidence_mask)
