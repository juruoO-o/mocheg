...
def train_loop(tuner_config, config_kwargs):
    ...
    dataloaders, train_label_statistic = get_data_loader(args.train_txt_dir, args.val_txt_dir, args.test_txt_dir,
                                                         args.style,
    ...
    model = TextModel(bert_model, args.concat_hidden_size, args.final_hidden_size, num_classes,
                      args.freeze_bert_layer_number).to(device)
    # freeze(clip_model)
    ...
    train(model, dataloaders, device, optimizer, args, train_label_statistic, logger, args.batch_size, scheduler,
          args.accum_iter)
...
def train(model, dataloaders, device, optimizer, args, train_label_statistic, logger, batch_size, scheduler,
          accum_iter):
    best_valid_f1 = 0
    best_epoch = 0
    # Loss function
